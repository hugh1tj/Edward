...
def premiums_alt_sub_sub(window, canvas, ship_list_me, ship_list_selected, insurers_list, from_index):
    ...
    fail_round_accum=0
    premium_below=-1
    uninsured_ships_transferred=False
    ### 3. COLOR DEFINITIONS ###
    color_text = ('black')
    color_border = ('blue')
    color_wash = ('white')
    ...
    title2b_text = font22.render("Premiums Agreed", True, color_text)
    ...
    table2c_start_y = table1_start_y + (smax + 6) * cell_height  # uninsured ships
    ...
    title1_start_y = table1_start_y-title_dif  # premiums offered
    title2b_start_y = table2b_start_y-title_dif  # premiums accepted
    title2c_start_y = table2c_start_y-title_dif  # uninsured ships
    title3_start_y = table3_start_y-title_dif # same y for residual book value
    title3_start_x = 1000
    title2c_start_x=800
    ...
    pygame.draw.rect(canvas, color_border, title2b_text_rect, 1)
    canvas.blit(title2b_text, title2b_text_rect)
    ...
    pygame.draw.rect(canvas, color_border, title2c_text_rect, 1)
    canvas.blit(title2c_text, title2c_text_rect)
    # cell_width_t2=size_width/(mmax*2)
    cell_width_t2c = cell_width_t1
    ...
    while (accepted_total+unins_numb)<smax:
        ...
        while accepted!=2 or accepted_total<=smax:
            ...
            # print ("------------------------------------------------------------------")
            if fail_round==False: # bidding was successful 
                #fail_round_accum=0
                #print("490 fail round accum reset - m",m)
                ...
                for sm in range (0,smax):
                    #print ("506 in fail_round is true - sm",sm,"ship name",premiums_offered_nested_list[sm + 2][m * 4],"insurer",m)
                    if premiums_offered_nested_list[sm + 2][m * 4] !="": # finds length of current offered list
                        smmax=sm
                    #print ("509 in fail_round is true - smmax",smmax,"ship name at smmax",premiums_offered_nested_list[smmax + 2][m * 4],"insurer",m)
                if s<smmax:
                    s+=1
            ...
            if fail_round_accum>=3: # test for uninsured ships
                ...
                for sunins in range(0,smmax+1): # because range function stop smmax+1 is exclusive
                    ...
                    for sfind in range(0, len(ship_list_selected)):
                        if ship_list_selected[sfind].ship_name == ship_name_search:
                            ship_list_selected[sfind].ship_insurer = "Uninsured"
                            ship_list_selected[sfind].ship_premium = 0
                            break
